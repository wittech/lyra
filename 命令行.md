...
PYTHONPATH=. python -m lyra_2._src.inference.lyra2_zoomgs_inference \
  ...
  --prompt_dir assets/samples \
  --output_path outputs/zoomgs \
  --num_frames_zoom_in 161 \
  --num_frames_zoom_out 81 \
  --zoom_in_strength 0.7 \
  --zoom_out_strength 0.6 \
  --use_dmd

值越大：镜头离初始视角越远，露出的新区域越多，运动更强
值越小：镜头变化更保守，新扩展区域更少，画面更稳定
zoom_out_strength=1.5 对室内图很容易扩出大量未见墙面和物件，文字、门窗、家具都容易乱。室内建议先从 0.5-0.8 试。
0       基本不做 zoom-out，或运动极弱
0.3     很轻微扩展
0.5-0.8 室内场景比较稳
1.0     中等扩展
1.5     默认示例强度，扩展明显
2.0+    很激进，容易 hallucination、结构漂移、文字乱码、物体变形
...
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
PYTHONPATH=. python -m lyra_2._src.inference.lyra2_custom_traj_inference \
  --input_image_path assets/custom_trajectory_examples/example_2/first_frame.png \
  --trajectory_path assets/custom_trajectory_examples/example_2/trajectory.npz \
  --experiment lyra2 \
  --checkpoint_dir checkpoints/model \
  --captions_path assets/custom_trajectory_examples/example_2/captions.json \
  --num_frames 481 \
  --output_path outputs/custom_traj \
  --use_dmd
