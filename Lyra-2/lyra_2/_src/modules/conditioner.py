...
from lyra_2._ext.imaginaire.functional.batch_ops import batch_mul
from lyra_2._ext.imaginaire.lazy_config import instantiate
from lyra_2._ext.imaginaire.utils import log
from lyra_2._ext.imaginaire.utils.count_params import count_params, disabled_train
from lyra_2._ext.imaginaire.utils.easy_io import easy_io
try:
    from lyra_2._src.datasets.data_sources.item_datasets_for_validation import get_itemdataset_option_local
except ImportError:
    get_itemdataset_option_local = None
...
class DataType(str, Enum):
    ...
    VIDEO = "video"
    MIX = "mix"
    COUPLED = "coupled"  # for coupled dataloader, video and image data are in one batch and concat together
...
class TextAttrEmptyStringDrop(AbstractEmbModel):
    ...
    def random_dropout_input(
        ...
        if self.empty_prompt_data is None:
            if get_itemdataset_option_local is None:
                raise ImportError(
...
class GeneralConditioner(nn.Module, ABC):
    ...
    def get_condition_with_negative_prompt(
        ...
        for emb_name, embedder in self.embedders.items():
            cond_dropout_rates[emb_name] = 0.0
            if isinstance(embedder, TextAttr) or isinstance(embedder, TextAttrEmptyStringDrop):
